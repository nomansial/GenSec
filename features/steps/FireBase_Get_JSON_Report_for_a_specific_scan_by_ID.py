...
@given(u'FireBase_User sends GET request to fetch JSON report for scan ID "{scan_id}"')
def step_impl(context, scan_id):
    """Send GET request to fetch JSON report for a given scan ID"""
    config = getConfig()
    context.scan_id = scan_id
    context.url = f"{config['API']['FireBaseURL']}/scans/{scan_id}/json"
...
@when(u'FireBase_the user receives the JSON report response')
def step_impl(context):
    """Ensure the JSON report response is received"""
    assert_that(context.response).is_not_none()

@then(u'FireBase_the status code returned should be 200 for successful report retrieval')
def step_impl(context):
    """Verify status code is 200"""
    assert_that(context.response.status_code).is_equal_to(200)

@then(u'FireBase_chatboatname should contain a "chatbot_name"')
def step_impl(context):
...
@then(u'FireBase_the response should contain a valid "scan_id" matching "{scan_id}"')
def step_impl(context, scan_id):
...
@then(u'FireBase_the response should contain a "start_timestamp" in ISO 8601 format')
def step_impl(context):
...
@then(u'FireBase_the response should contain an "end_timestamp" in ISO 8601 format')
def step_impl(context):
...
@then(u'FireBase_the response should contain a list of findings')
def step_impl(context):
...
@then(u'FireBase_each finding should contain:')
def step_impl(context):
...
@then(u'FireBase_the response should contain the error "Bad Request"')
def step_impl(context):
...
@then(u'FireBase_the response should contain the error message "Invalid input:"')
def step_impl(context):
    """Check if the response contains the specified error message"""
    response_json = context.response.json()
    assert_that(response_json.get("message")).contains('Invalid input:')

@then(u'FireBase_the response should contain the message "The current request is not defined by this API."')
def step_impl(context):
