...
@given(u'FireBase_User sends GET request to fetch chatbots with env_id "{env_id}"')
def step_impl(context, env_id):
    """Send GET request to fetch chatbots for a given env_id"""
    config = getConfig()
    context.url = f"{config['API']['FireBaseURL']}/chatbots?env_id={env_id}"
...
@then(u'FireBase_the status code returned should be 200 for a successful request')
...
@then(u'FireBase_the response should contain a "chatbots" array')
def step_impl(context):
...
@then(u'FireBase_the "chatbots" array should contain at least one chatbot')
def step_impl(context):
...
@then(u'FireBase_the response should contain chatbot details including "chatbot_id" and "name"')
def step_impl(context):
...
@then(u'FireBase_error code should be 400 for bad request error')
def step_impl(context):
...
@then(u'FireBase_response shows error message "Bad Request"')
def step_impl(context):
...
@then(u'FireBase_the message should be "Invalid input:"')
def step_impl(context):
