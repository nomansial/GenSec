...
@given(u'FireBase_User sends PUT request to update chatbot API details for chatbot ID with environment ID and endpoint "{chatbot_url}"')
...
@given(u'FireBase_User sends PUT request to update chatbot API details for chatbot ID "{chatbot_id}" with environment ID "{env_id}" and endpoint "{chatbot_url}"')
...
@when(u'FireBase_the user receives the chatbot API update response')
def step_impl(context):
    assert_that(context.response).is_not_none()

@then(u'FireBase_the status code returned should be 200 for successful update')
def step_impl(context):
    assert_that(context.response.status_code).is_equal_to(200)


@then(u'FireBase_the response should contain a valid chatbotID')
def step_impl(context):
...
@then(u'FireBase_the response should contain a "status" equal to "API_updated"')
def step_impl(context):
    response_json = context.response2.json()
    assert_that(response_json.get("status")).is_equal_to("API_updated")


@then(u'FireBase_should be 404 for not found error')
def step_impl(context):
    assert_that(context.response.status_code).is_equal_to(404)

@then(u'FireBase_should error message "Not Found"')
def step_impl(context):
    error_message = context.response.json().get("error")
    assert_that(error_message).is_equal_to("Not Found")

@then(u'FireBase_the message should contain "Chatbot with ID {chatbot_id} not found."')
...
@given(
    u'FireBase_User send request to update API "{chatbot_id}" with environment ID "{env_id}" and endpoint "{chatbot_url}"')
def step_impl(context, chatbot_id, env_id, chatbot_url):
...
@when(u'FireBase_the user receives the API response')
def step_impl(context):
    """The response is already captured in context.response during the PUT request"""
    assert_that(context.response).is_not_none()


@then(u'FireBase_message should contain "Invalid input:"')
def step_impl(context):
...
@then(u'FireBase_the message should contain "chatbot_id" with validation errors related to the input')
def step_impl(context):
