...
@given(u'User sends GET request to fetch findings for scan ID "{scan_id}"')
def step_impl(context, scan_id):
    config = getConfig()
    context.scan_id = scan_id
    context.url = f"{config['API']['BaseURL']}/findings?scan_id={scan_id}"
...
@when(u'the user receives the findings response')
def step_impl(context):
    assert_that(context.response).is_not_none()

@then(u'the status code returned should be 200 for successful findings retrieval')
def step_impl(context):
    assert_that(context.response.status_code).is_equal_to(200)

@then(u'the response contain a valid "chatbot_id" of length 36')
def step_impl(context):
...
@then(u'the response should contain a non-empty "chatbot_name"')
def step_impl(context):
...
@then(u'the "scan_id" in response should match "{scan_id}"')
def step_impl(context, scan_id):
    actual_scan_id = context.response.json().get("scan_id")
    assert_that(actual_scan_id).is_equal_to(scan_id)

@then(u'the response should contain a non-empty list of findings')
def step_impl(context):
...
@then(u'each finding should include the following fields:')
def step_impl(context):
...
@then(u'each finding with a "creation_timestamp" should be in ISO 8601 format')
def step_impl(context):
...
@then(u'the response should contain null "chatbot_id" and "chatbot_name"')
def step_impl(context):
    response_json = context.response.json()
    assert_that(response_json.get("chatbot_id")).is_none()
    assert_that(response_json.get("chatbot_name")).is_none()

@then(u'the response should contain an empty list of findings')
def step_impl(context):
    findings = context.response.json().get("findings")
    assert_that(findings).is_instance_of(list)
