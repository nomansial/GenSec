...
@given(u'FireBase_User sends PUT request to update chatbot details with chatbot_name "{chatbot_name}", chatbot_description "{chatbot_description}", and env_id "{env_id}"')
def step_impl(context, chatbot_name, chatbot_description, env_id):
    ...
    config = getConfig()
    base_url = config['API']['FireBaseURL']
    ...
    for attempt in range(max_retries):
        status_response = requests.get(status_url, headers=context.headers)
...
@then(u'FireBase_the response should contain the chatbot_id')
...
@then(u'FireBase_the status should be In_progress')
def step_impl(context):
...
@given(u'FireBase_User sends PUT request to update chatbot details with chatbot_id "{chatbot_id}", chatbot_name "{chatbot_name}", chatbot_description "{chatbot_description}", and env_id "{env_id}"')
def step_impl(context, chatbot_id, chatbot_name, chatbot_description, env_id):
    ...
    config = getConfig()
    context.url = f"{config['API']['FireBaseURL']}/chatbots/{chatbot_id}"
...
@then(u'FireBase_the status code returned should be 400')
def step_impl(context):
    """Verify that the status code is 400 for a bad request"""
    assert_that(context.response.status_code).is_equal_to(400)

@then(u'FireBase_the response should contain error variable with value Bad Request')
def step_impl(context):
...
@then(u'FireBase_the response should contain the The chatbot must be in AVAILABLE status')
def step_impl(context):
...
@given(u'FireBase_User sends request with chatbot_id "{chatbot_id}", chatbot_name "{chatbot_name}", chatbot_description "{chatbot_description}", and env_id "{env_id}"')
def step_impl(context, chatbot_id, chatbot_name, chatbot_description, env_id):
...
@then(u'FireBase_the response should contain error variable The current request is matched to the defined')
def step_impl(context):
    """Verify that the response contains the 'Bad Request' error"""
    response_json = context.response.json()
    logger.info(f"Response contains error which is: {response_json.get('message')}")
    assert_that(response_json['message']).contains("The current request is matched to the defined")


@then(u'FireBase_the response should contain invalid input message')
def step_impl(context):
    """Verify that the response contains the message 'Invalid input:'"""
    response_json = context.response.json()
    logger.info(f"Response message: {response_json.get('message')}")
    assert_that(response_json['message']).contains("The current request is matched to the defined url template")