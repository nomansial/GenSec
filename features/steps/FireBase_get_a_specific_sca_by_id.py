...
@given(u'FireBase_User sends GET request to fetch scan details for scan ID "{scan_id}"')
def step_impl(context, scan_id):
    """Send GET request to fetch scan details"""
    config = getConfig()
    context.scan_id = scan_id
    context.url = f"{config['API']['FireBaseURL']}/scans/{scan_id}"
...
@when(u'FireBase_the user receives the scan details response')
def step_impl(context):
    """Ensure the scan details response is received"""
    assert_that(context.response).is_not_none()

@then(u'FireBase_the status code returned should be 200 for successful scan details retrieval')
def step_impl(context):
    """Verify status code is 200"""
    assert_that(context.response.status_code).is_equal_to(200)

@then(u'FireBase_the response should contain a valid "chatbot_id" of length 36')
def step_impl(context):
...
@then(u'FireBase_the response should contain a "chatbot_name"')
def step_impl(context):
...
@then(u'FireBase_the "creation_timestamp" should be in ISO 8601 format')
def step_impl(context):
...
@then(u'FireBase_the "last_update_timestamp" should be in ISO 8601 format')
def step_impl(context):
...
@then(u'FireBase_the "scan_id" should match the requested scan ID "{scan_id}"')
def step_impl(context, scan_id):
    """Verify scan_id matches the requested one"""
    response_json = context.response.json()
    assert_that(response_json.get("scan_id")).is_equal_to(scan_id)

@then(u'FireBase_the "status" should be "SCAN_COMPLETED"')
def step_impl(context):
...
@then(u'FireBase_the message should contain "Scan with ID {scan_id} not found."')
def step_impl(context, scan_id):
...
@given(u'FireBase_Specific Scan by ID the user sends GET request with finding ID')
def step_impl(context):
    ...
    context.url = f"{config['API']['FireBaseURL']}/scans/{finding_id}"  # Corrected to 'scans'
...
@then(u'FireBase_Specific Scan by ID the status code returned should be 404')
def step_impl(context):
    assert_that(context.response.status_code).is_equal_to(404)

@then(u'FireBase_Specific Scan by ID the message should contain "The current request is not defined by this API."')
def step_impl(context):
    response_json = context.response.json()
