...
@given(u'User sends GET request to fetch scans for chatbot ID "{chatbot_id}"')
def step_impl(context, chatbot_id):
    """Send GET request to fetch scans for a given chatbot ID"""
    config = getConfig()
    context.chatbot_id = chatbot_id
    context.url = f"{config['API']['BaseURL']}/scans?chatbot_id={chatbot_id}"
...
@when(u'the user receives the scans response')
def step_impl(context):
    """Ensure the scans response is received"""
    assert_that(context.response).is_not_none()

@then(u'the status code returned should be 200 for successful scans retrieval')
def step_impl(context):
    """Verify status code is 200"""
    assert_that(context.response.status_code).is_equal_to(200)

@then(u'the response should contain a list of scans')
def step_impl(context):
...
@then(u'each scan should contain a valid "scan_id" of length 36')
def step_impl(context):
...
@then(u'each scan should contain a "creation_timestamp" in ISO 8601 format')
def step_impl(context):
...
@then(u'each scan should have a "status" equal to "REPORT_GENERATED"')
def step_impl(context):
