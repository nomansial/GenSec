...
@given(u'FireBase_User sends GET request to fetch customer API key')
def step_impl(context):
    """Send GET request to fetch customer API key"""
    config = getConfig()
    context.url = f"{config['API']['FireBaseURL']}/customers/me/api"
...
@then(u'FireBase_the status code returned should be 200 for a successful request for get customer API')
def step_impl(context):
    """Verify status code 200"""
    assert_that(context.response.status_code).is_equal_to(200)

@then(u'FireBase_the response should contain the customer API key details')
def step_impl(context):
...
@then(u'FireBase_all the parameters in the response should not be None')
def step_impl(context):
...
@given(u'FireBase_User sends GET request to fetch customer API key with invalid or missing API key')
def step_impl(context):
...
@then(u'FireBase_the status code returned should be 401 for unauthorized request')
def step_impl(context):
    """Verify status code 401"""
    assert_that(context.response.status_code).is_equal_to(401)

@then(u'FireBase_the response should contain an error message for customer API')
def step_impl(context):
...
@then(u'FireBase_the error message should state "Invalid or missing API key"')
def step_impl(context):
    """Verify specific unauthorized error message"""
    response_json = context.response.json()
    expected_msg = "Jwt is missing"
