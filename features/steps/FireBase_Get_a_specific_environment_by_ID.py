...
@given(u'FireBase_take env_id from Create Environment API')
...
@when(u'FireBase_user sends GET request to fetch environment details with valid env_id')
def step_impl(context):
    """Send GET request to fetch environment details using valid env_id from context"""
    config = getConfig()
    context.url = f"{config['API']['FireBaseURL']}/environments/{context.env_id}"  # Correct URL format
...
@then(u'FireBase_the status code should be 200 for the successful request')
def step_impl(context):
...
@then(u'FireBase_the response should contain the env_id')
def step_impl(context):
...
@then(u'FireBase_the "env_id" should match the env_id fetched from Create Environment API')
def step_impl(context):
...
@then(u'FireBase_the "creation_timestamp" should be in the correct timestamp format (yyyy-MM-dd\'T\'HH:mm:ss\'Z\')')
def step_impl(context):
...
@then(u'FireBase_the "last_update_timestamp" should be in the correct timestamp format (yyyy-MM-dd\'T\'HH:mm:ss\'Z\')')
def step_impl(context):
...
@then(u'FireBase_the "description" should not be null')
def step_impl(context):
...
@then(u'FireBase_the "name" should not be null')
def step_impl(context):
...
@given(u'FireBase_User sends GET request to fetch environment details with an invalid or missing env_id "{env_id}"')
def step_impl(context, env_id):
    """Send GET request to fetch environment details using invalid env_id (non-existing or blank)"""
    config = getConfig()
    context.url = f"{config['API']['FireBaseURL']}/environments/{env_id}"
...
@then(u'FireBase_the status code should be 400 for not found error')
def step_impl(context):
...
@then(u'FireBase_the response should contain an error message')
def step_impl(context):
...
@given(u'FireBase_User sends GET request to fetch environment details with an invalid env_id "{env_id}"')
def step_impl(context, env_id):
...
@then(u'FireBase_API should contain the message Current request is not defined by this API')
def step_impl(context):
