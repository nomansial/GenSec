...
@given(u'User sends GET request to fetch finding details for finding ID "{finding_id}"')
...
@when(u'the user receives the finding details response')
def step_impl(context):
    assert_that(context.response).is_not_none()

@then(u'the status code returned should be 200 for successful finding retrieval')
def step_impl(context):
    assert_that(context.response.status_code).is_equal_to(200)

@then(u'the response should contain a valid "finding_id" matching "{finding_id}"')
def step_impl(context, finding_id):
    response_json = context.response.json()
    assert_that(response_json.get("finding_id")).is_equal_to(finding_id)

@then(u'the response should contain a valid "scan_id" of length 36')
def step_impl(context):
...
@then(u'contain a non-empty "{field}"')
def step_impl(context, field):
    value = context.response.json().get(field)
    assert_that(value).is_not_none()
    assert_that(str(value)).is_not_empty()

@then(u'the response should contain a "creation_timestamp" in ISO 8601 format')
def step_impl(context):
...
@then(u'the response should contain non-empty lists for "data_classification" and "data_targeted"')
def step_impl(context):
    response_json = context.response.json()
...
@then(u'the message should contain "Finding with ID {finding_id} not found."')
def step_impl(context, finding_id):
